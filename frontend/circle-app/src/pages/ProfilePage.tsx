...
export default function ProfilePage() {
  ...
  const checkFollowStatus = async (profileUserId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:9000/api/user/${currentUser.id}/following`, {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:9000/api/user/${endpoint}`, {
        method: 'POST',
        headers: {
  ...
  const fetchStats = async (id: string) => {
    ...
    try {
      const [followersRes, followingRes] = await Promise.all([
        fetch(`http://localhost:9000/api/user/${id}/followers`, {
          headers: { Authorization: `Bearer ${token}` },
          credentials: 'include',
        }),
        fetch(`http://localhost:9000/api/user/${id}/following`, {
          headers: { Authorization: `Bearer ${token}` },
          credentials: 'include',
        })
      ]);
  ...
  const handleToggleThreadLike = async (threadId: number, hasLiked: boolean) => {
    ...
    try {
      ...
      const result = await dispatch(toggleLike({ threadId, currentIsLiked: hasLiked }));
  ...
  src={
    profileUser.photo_profile
      ? `http://localhost:9000/uploads/${profileUser.photo_profile}`
      : "/default-avatar.png"
  }
