...
export default function SearchBar() {
  ...
  const searchSuggestions = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:9000/api/user/search?q=${encodeURIComponent(searchQuery)}`, {
        headers: {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [searchResponse, followingResponse] = await Promise.all([
        fetch(`http://localhost:9000/api/user/search?q=${encodeURIComponent(query)}`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
          credentials: 'include'
        }),
        currentUser?.id ? fetch(`http://localhost:9000/api/user/${currentUser.id}/following`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
          credentials: 'include'
